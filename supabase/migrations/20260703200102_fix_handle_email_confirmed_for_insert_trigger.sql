-- OLD is not assigned in an AFTER INSERT trigger — referencing old.email_confirmed_at
-- unconditionally (as the previous version did) would raise
-- "record 'old' is not assigned yet" the first time the new INSERT trigger
-- fires. Check tg_op first so OLD is never touched on INSERT.
